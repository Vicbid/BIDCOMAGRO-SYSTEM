  ...
  try {
    ...
    for (var i = 1; i < dSol.length; i++) {
      ...
      if (!porOT[ot]) porOT[ot] = {
        ot: ot, reseller: String(f[3]), urgencia: String(f[9]),
        fecha: _fmtFecha(f[1]),
        diasEspera: (f[1] instanceof Date) ? Math.floor((hoy-f[1])/86400000) : 0,
        items: []
      };
      var cod2 = String(f[4]).trim().toUpperCase();
      var info = stockMap[cod2] || { stock:0, ubicacion:"" };
      porOT[ot].items.push({
        fila: i+1, id: String(f[0]), codigo: cod2,
        descripcion: String(f[5]), cantSolicitada: parseInt(f[6])||0,
        cantDespachada: parseInt(f[7])||0, estado: est,
        stockDisponible: info.stock, ubicacion: info.ubicacion
      });
    }

    var out = [];
    var keys = Object.keys(porOT);
    for (var k = 0; k < keys.length; k++) out.push(porOT[keys[k]]);
    out.sort(function(a,b){ return b.diasEspera - a.diasEspera; });
    return out;
  } catch(e) { Logger.log("cargarSolicitudesDespacho: "+e); return []; }
}

// Cancela filas Pendiente en SOLICITUDES_DESPACHO cuya OT ya está Finalizada/Entregada/Cancelada.
// Ejecutar manualmente una vez para limpiar datos existentes; también útil como mantenimiento.
function limpiarSolicitudesDeOTsCerradas() {
  try {
    var ESTADOS_TERMINAL = ['Finalizado', 'Entregado', 'CANCELADO'];
    var dOT  = getSheetValues(SCHEMA.SHEETS.OT);
    var otsCerradas = {};
    for (var o = 1; o < dOT.length; o++) {
      var est = String(dOT[o][SCHEMA.OT.ESTADO] || '').trim();
      if (ESTADOS_TERMINAL.indexOf(est) !== -1) {
        otsCerradas[String(dOT[o][SCHEMA.OT.OT] || '').trim().toUpperCase()] = true;
      }
    }

    var hojaSol = getSheet(SCHEMA.SHEETS.SOLICITUDES);
    if (!hojaSol) return { cerradas: 0 };
    ...
    for (var i = 1; i < datos.length; i++) {
      ...
      var otRow = String(datos[i][SD.OT] || '').trim().toUpperCase();
      if (otsCerradas[otRow]) {
        datos[i][SD.ESTADO]         = 'Cancelado';
        datos[i][SD.FECHA_DESPACHO] = hoy;
        datos[i][SD.OPERADOR]       = 'Sistema (limpieza)';
        cerradas++;
      }
    }
    if (cerradas) {
  ...
  } catch(e) { Logger.log('limpiarSolicitudesDeOTsCerradas: ' + e); return { error: e.message }; }
}

function generarReportePendientesPDF() {
  try {
    var SD = SCHEMA.SOLICITUDES_DESPACHO;
    var S  = SCHEMA.STOCK_REPUESTOS;
    var dSol   = getSheetValues(SCHEMA.SHEETS.SOLICITUDES);
    ...
    for (var s = 1; s < dStock.length; s++) {
      ...
      if (cod) ubicMap[cod] = String(dStock[s][S.UBICACION] || '—');
    }

    // Leer TABLA_POSICIONES para asignar bin primario (menor orden) a cada SKU
    var binPrimarioMap = {};
    try {
      var dPos2 = getSheetValues(SCHEMA.SHEETS.TABLA_POSICIONES);
      var TP2   = SCHEMA.TABLA_POSICIONES;
      var skuBins = {};
      for (var pb = 1; pb < dPos2.length; pb++) {
        var pbSku = String(dPos2[pb][TP2.SKU] || '').trim().toUpperCase();
        if (!pbSku) continue;
        var pbBin = String(dPos2[pb][TP2.BIN_ID] || '').trim();
        if (!pbBin) continue;
        if (!skuBins[pbSku]) skuBins[pbSku] = [];
        skuBins[pbSku].push(pbBin);
      }
      var skuKeys = Object.keys(skuBins);
      for (var sk = 0; sk < skuKeys.length; sk++) {
        var skuK = skuKeys[sk];
        skuBins[skuK].sort(function(a, b) {
          return _binSortKey(a) < _binSortKey(b) ? -1 : 1;
        });
        binPrimarioMap[skuK] = skuBins[skuK][0];
      }
    } catch(epb) {}
    ...
    for (var i = 1; i < dSol.length; i++) {
      var f   = dSol[i];
      var est = String(f[SD.ESTADO] || '');
      if (est === 'Despachado' || est === 'Cancelado') continue;
      var sku  = String(f[SD.CODIGO] || '').trim().toUpperCase();
      var pend = (parseInt(f[SD.CANT_SOLICITADA]) || 0) - (parseInt(f[SD.CANT_DESPACHADA]) || 0);
      if (pend <= 0) continue;
      // Usar bin WMS si existe, si no fallback a ubicación estática
      var binPrim = binPrimarioMap[sku];
      var ubicLabel = binPrim || ubicMap[sku] || '—';
      items.push({
        ot:       String(f[SD.OT]          || '—'),
        sku:      sku,
        desc:     String(f[SD.DESCRIPCION] || ''),
        cant:     pend,
        ubic:     ubicLabel,
        sortKey:  binPrim ? _binSortKey(binPrim) : (ubicLabel === '—' ? 'zzz' : ubicLabel)
      });
    }

    if (!items.length) return { vacio: true };

    // Consolidar por SKU sumando cantidades de distintas OTs
    var consolMap = {};
    for (var c2 = 0; c2 < items.length; c2++) {
      var ci = items[c2];
      if (!consolMap[ci.sku]) {
        consolMap[ci.sku] = { ots: [], sku: ci.sku, desc: ci.desc,
          cant: 0, ubic: ci.ubic, sortKey: ci.sortKey };
      }
      consolMap[ci.sku].cant += ci.cant;
      if (consolMap[ci.sku].ots.indexOf(ci.ot) === -1) consolMap[ci.sku].ots.push(ci.ot);
    }
    items = [];
    for (var ck in consolMap) items.push(consolMap[ck]);

    // Ordenar por Ruta de Recolección: Pasillo → Estante → Nivel
    items.sort(function(a, b) {
      if (a.sortKey === 'zzz' && b.sortKey === 'zzz') return 0;
      if (a.sortKey === 'zzz') return 1;
      if (b.sortKey === 'zzz') return -1;
      return a.sortKey < b.sortKey ? -1 : (a.sortKey > b.sortKey ? 1 : 0);
    });

    var tz      = Session.getScriptTimeZone();
    var docName = 'PickingList_' + Utilities.formatDate(new Date(), tz, 'yyyyMMdd_HHmm');
    var doc     = DocumentApp.create(docName);
    var body    = doc.getBody();
    body.clear();

    body.appendParagraph('BIDCOMAGRO — PICKING LIST')
        .setHeading(DocumentApp.ParagraphHeading.HEADING1)
        .setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    body.appendParagraph('Solicitudes pendientes de despacho · ' +
    ...
    var allRows = [['#', 'OTs', 'SKU', 'Descripción', 'Cant. total', 'Bin / Ubicación']];
    for (var k = 0; k < items.length; k++) {
      var it = items[k];
      allRows.push([String(k + 1), it.ots.join(', '), it.sku, it.desc, String(it.cant), it.ubic]);
    }

    var table = body.appendTable(allRows);
    var hdr = table.getRow(0);
    for (var c = 0; c < hdr.getNumCells(); c++) {
      hdr.getCell(c).setFontSize(9).setBold(true).setBackgroundColor('#d0e8ff');
    }
    for (var r = 1; r < table.getNumRows(); r++) {
      var row = table.getRow(r);
      for (var c2 = 0; c2 < row.getNumCells(); c2++) {
        row.getCell(c2).setFontSize(9);
      }
    }

    body.appendParagraph('');
    body.appendParagraph('Total: ' + items.length + ' SKUs consolidados')
        .setFontSize(9).setItalic(true);
    body.appendParagraph('Generado el ' + Utilities.formatDate(new Date(), tz, 'dd/MM/yyyy HH:mm') +
        '  ·  BIDCOMAGRO  ·  Stock Manager')
        .setFontSize(8)
        .setAlignment(DocumentApp.HorizontalAlignment.CENTER);
    ...
    return { base64: base64, nombre: docName };
  } catch(e) {
    Logger.log('generarReportePendientesPDF: ' + e);
    return { error: e.message };
  }
}

function procesarDespacho(items, operador) {
  var lock = LockService.getScriptLock();
  try {
    lock.waitLock(30000); 

    var ss = getSS();
    var hojaSD = ss.getSheetByName(SCHEMA.SHEETS.SOLICITUDES);
    var hojaStr = ss.getSheetByName(SCHEMA.SHEETS.STOCK);
    var hoy = new Date();

    var dSD = hojaSD.getDataRange().getValues();
    var dStr = hojaStr.getDataRange().getValues();

    var stockIndex = {};
    for (var s = 1; s < dStr.length; s++) {
      stockIndex[String(dStr[s][0]).trim().toUpperCase()] = {
        fila: s + 1,
        stock: parseInt(dStr[s][2]) || 0,
        descripcion: String(dStr[s][1] || "")
      };
    }

    var resultados = [];
    var ot = items.length ? String(items[0].ot || "") : "";
    var reseller = items.length ? String(items[0].reseller || "") : "";
    var notificar = false;
    var stockModificado = false;

    for (var i = 0; i < items.length; i++) {
      var item = items[i] || {};
      var fila = parseInt(item.fila);
      if (isNaN(fila) || fila < 2 || fila > dSD.length) {
        resultados.push({ codigo: item.codigo || "", accion: "error", msg: "Fila inválida" });
        continue;
      }

      var row = dSD[fila - 1];
      if (!row) {
        resultados.push({ codigo: item.codigo || "", accion: "error", msg: "Solicitud no encontrada" });
        continue;
      }

      var cantSol  = parseInt(row[6]) || 0;
      var cantDesp = parseInt(row[7]) || 0;
      var accion   = String(item.accion || "despachar").toLowerCase();
      var qty      = parseInt(item.cantDespachar) || 0;

      if (accion === "cancelar") {
        dSD[fila - 1][8]  = "Cancelado";
        dSD[fila - 1][10] = hoy;
        resultados.push({ codigo: item.codigo || "", accion: "cancelado" });
        continue;
      }

      if (qty <= 0) {
        resultados.push({ codigo: item.codigo || "", accion: "omitido", msg: "Cantidad debe ser mayor a 0" });
        continue;
      }

      var codKey    = String(item.codigo || "").trim().toUpperCase();
      var stockInfo = stockIndex[codKey];
      if (!stockInfo) {
        resultados.push({ codigo: item.codigo || "", accion: "error", msg: "Código no encontrado en stock" });
        continue;
      }

      var restante  = Math.max(cantSol - cantDesp, 0);
      var despachar = 0;

      despachar = Math.min(qty, restante);
      if (despachar <= 0) {
        resultados.push({ codigo: item.codigo || "", accion: "sin_cantidad", msg: "No queda cantidad pendiente" });
        continue;
      }

      var nuevoDesp  = cantDesp + despachar;
      var estadoNuevo = nuevoDesp >= cantSol ? "Despachado" : "Pendiente";
      dSD[fila - 1][7]  = nuevoDesp;
      dSD[fila - 1][8]  = estadoNuevo;
      dSD[fila - 1][10] = hoy;

      resultados.push({ codigo: item.codigo, accion: "despachado", despachado: despachar });
      notificar = true;
    }

    if (stockModificado) {
      hojaStr.getDataRange().setValues(dStr);
      // Sincronizar Carmen: solo los SKUs efectivamente despachados
      var _sincMap = {};
      for (var _ri = 0; _ri < resultados.length; _ri++) {
        if (resultados[_ri].accion !== 'despachado') continue;
        var _rCod = String(resultados[_ri].codigo || '').trim().toUpperCase();
        if (_rCod && !_sincMap[_rCod] && stockIndex[_rCod]) {
          _sincMap[_rCod] = true;
          _actualizarCarmenStock(_rCod, stockIndex[_rCod].stock);
        }
      }
    }
    hojaSD.getDataRange().setValues(dSD);
    SpreadsheetApp.flush();
    if (notificar && ot) _notificarDespacho(ot, reseller, resultados);
    return { ok: true, resultados: resultados };

  } catch (e) {
    return { ok: false, msg: "Error al procesar despacho: " + e.toString() };
  } finally {
    try { if (lock.hasLock()) lock.releaseLock(); } catch (e) {}
  }
}

function _actualizarEnviadosEnRep(repStr, codigo, cantDespachar) {
  if (!repStr || repStr === "Sin consumo de repuestos") return repStr;
  var items = repStr.split(" ; ");
  for (var i = 0; i < items.length; i++) {
    var p = items[i].split(" | ");
    if (p.length < 3) continue;
    if (p[0].trim().toUpperCase() === codigo.toUpperCase()) {
      var ped = parseInt(p[2].split(" E:")[0].replace("P:",""))||0;
      var env = parseInt(p[2].split(" E:")[1])||0;
      var nuevoEnv = Math.min(ped, env + cantDespachar);
      p[2] = "P:" + ped + " E:" + nuevoEnv;
      items[i] = p.join(" | ");
    }
  }
  return items.join(" ; ");
...
function _notificarDespacho(ot, reseller, resultados) {
  try {
    var filas = resultados.map(function(r) {
      if (r.accion==="cancelado") return "<tr><td style='padding:6px 10px;font-size:12px'>"+r.codigo+"</td><td style='padding:6px 10px;font-size:12px;color:#e74c3c'>Cancelado</td></tr>";
      return "<tr><td style='padding:6px 10px;font-size:12px'>"+r.codigo+"</td><td style='padding:6px 10px;font-size:12px;color:#27ae60'>Despachado: "+r.despachado+" u.</td></tr>";
    }).join("");
    var html =
      "<div style='font-family:sans-serif;max-width:500px'>" +
      "<div style='background:#00a3e0;padding:16px 20px;border-radius:8px 8px 0 0'>" +
        "<span style='color:#fff;font-size:16px;font-weight:700'>Stock Manager — Despacho confirmado</span>" +
      "</div>" +
      "<div style='background:#fff;border:1px solid #ddd;padding:18px 20px;border-radius:0 0 8px 8px'>" +
        "<p style='font-size:13px;color:#444;margin:0 0 14px'>La OT <strong>"+ot+"</strong> ("+reseller+") recibió un despacho.</p>" +
        "<table style='width:100%;border-collapse:collapse;border:1px solid #eee'>" +
          "<thead><tr style='background:#f5f5f5'><th style='padding:6px 10px;font-size:11px;text-align:left'>Código</th><th style='padding:6px 10px;font-size:11px;text-align:left'>Resultado</th></tr></thead>" +
          "<tbody>"+filas+"</tbody>" +
        "</table>" +
        "<p style='font-size:11px;color:#999;margin-top:14px'>Revisá la OT en el DJI HUB PRO para ver el estado actualizado.</p>" +
      "</div></div>";
    GmailApp.sendEmail(SM_CONFIG.EMAIL_SUPERVISOR,
      "[Stock Manager] Despacho confirmado — " + ot, "", {
        htmlBody: html, name: SM_CONFIG.NOMBRE_REMITENTE,
        replyTo: SM_CONFIG.EMAIL_SUPERVISOR
      });
  } catch(e) { Logger.log("_notificarDespacho: "+e); }
}
...
//  Archivo principal (stub). Lógica distribuida en:
//    Sm_Core.js       — variables, Carmen helpers, utilidades
//    Sm_Dashboard.js  — cargarDashboard, OTs, rotación
//    Sm_Stock.js      — stock CRUD, inventario, clasificación, reservas
